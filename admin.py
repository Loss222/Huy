...
def register_admin(db, bot: Bot, admin_ids: List[int], platform_fee: int = 99):
    """
    ...
    # ФИКС: Удаляем конфликтующие хендлеры
    # router.message.filter(F.text != BTN_ADMIN)  # Игнорируем кнопку админки в основном роутере
    ...
    def get_admin_main_kb():
        ...
        return InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [InlineKeyboardButton(text="📅 Список событий", callback_data=CB_ADMIN_EVENTS)],
    ...
    def get_admin_bookings_kb(bookings, current_page=0, total_pages=1):
        ...
        for booking in bookings:
            ...
            buttons.append([
                InlineKeyboardButton(
                    text=text,
                    callback_data=f"admin:booking_info:{booking_id}"
                )
            ])
    ...
    # === ОБРАБОТЧИКИ АДМИНКИ БЕЗ StateFilter ===
    ...
    @admin_router.callback_query(F.data == CB_ADMIN_EVENTS)
    async def admin_events_list_handler(callback: CallbackQuery, state: FSMContext):
        """Список всех событий"""
        if not await check_admin_access(callback.from_user.id):
            await callback.answer(ADMIN_NO_ACCESS)
            return
        
        await state.set_state(AdminStates.EVENTS_LIST)
    ...
    async def admin_event_detail_handler(callback: CallbackQuery, state: FSMContext):
        ...
        await state.set_state(AdminStates.EVENT_DETAIL)
    ...
    async def admin_bookings_handler(callback: CallbackQuery, state: FSMContext):
        ...
        await state.set_state(AdminStates.BOOKINGS)
    ...
    async def booking_info_handler(callback: CallbackQuery, state: FSMContext):
        ...
        try:
            booking_id = int(callback.data.split("admin:booking_info:", 1)[1])
        except Exception:
            await callback.answer("❌ Неверный ID брони")
            return

        await state.set_state(AdminStates.BOOKING_DETAIL)
    ...
    async def admin_bookings_page_handler(callback: CallbackQuery, state: FSMContext):
    ...
    # Хэндлер для перехода в админку из главного меню
    ...
    async def admin_callback_fallback(callback: CallbackQuery, state: FSMContext):
        """Fallback для необработанных админских callback'ов (только с префиксом admin:)"""
        # Если это админский callback, но не обработан
        await callback.answer("⚠️ Админский callback не обработан", show_alert=False)
        logging.warning(f"Unhandled admin callback: {callback.data}")
